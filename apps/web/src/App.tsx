...
function App() {
	...
	const [roundDurationSeconds, setRoundDurationSeconds] =
		useState<RoundDurationSeconds>(20);
	...
	const [nextRoundOwnerName, setNextRoundOwnerName] = useState<string | null>(
		null,
	);
	...
	const isYouTubeRound = Boolean(
		round?.audioUrl.includes("youtube.com/embed/"),
	);
	const roundTotalSeconds = round?.roundDurationSeconds ?? roundDurationSeconds;
	const elapsedRoundSeconds =
		playStartedAtMs === null ? 0 : Math.floor((nowMs - playStartedAtMs) / 1000);
	const remainingRoundSeconds = round
		? Math.max(0, roundTotalSeconds - elapsedRoundSeconds)
		: 0;
	const roundProgressPercent = round
		? Math.min(100, (remainingRoundSeconds / roundTotalSeconds) * 100)
		: 0;
	const isRoundTimeUp =
		Boolean(round) && playStartedAtMs !== null && remainingRoundSeconds <= 0;
	const allTitles = useMemo(
		() => (round ? round.options.map((option) => option.title) : []),
		[round],
	);
	...
	const loadRoomState = async (requestedPlayerId: string) => {
		const response = await fetch(
			`${apiUrl("/room/state")}?playerId=${encodeURIComponent(requestedPlayerId)}`,
		);
		if (!response.ok) {
	...
	const startNextRound = async () => {
		...
		try {
			...
			setValidationMessage(
				error instanceof Error ? error.message : "Unable to start next round",
			);
		}
	};
	...
	const toggleNativePlayback = async () => {
		...
		if (
			element.currentTime < OPENING_START_SECONDS &&
		...
		try {
			...
			setValidationMessage(
				payload.correct
					...
					: `Wrong. Correct answer: ${payload.correctOpeningTitle}`,
			);
			sendYouTubeCommand("pauseVideo");
			setYoutubePlaying(false);
			setNativePlaying(false);
	...
	const resetScores = async () => {
		...
		try {
			...
			setStatus("error");
			setValidationMessage(
				error instanceof Error ? error.message : "Unable to reset scores",
			);
		}
	};
		...
		<main className="app-shell">
			<section className="quiz-card battle-card">
				...
				<div className="player-wrap">
							...
							<>
								<div className="audio-controls">
									<label
										...
										htmlFor="round-duration-youtube"
									>
										Round timer
										<select
											...
											onChange={(event) =>
												setRoundDurationSeconds(
													Number(event.target.value) as RoundDurationSeconds,
												)
											}
									...
									<button
										...
										disabled={
											status !== "ready" || roundResolved || isRoundTimeUp
										}
									>
								...
								<div className="audio-controls">
									...
									<label className="volume-wrap" htmlFor="native-volume">
										...
										<input
											...
											value={nativeVolume}
											onChange={(event) =>
												setNativeVolume(Number(event.target.value))
											}
											disabled={status !== "ready"}
										/>
									</label>
					...
					{!canStartNextRound && nextRoundOwnerName && (
						<p className="muted selector-help">
							{nextRoundOwnerName} can start the next opening.
						</p>
					)}
					{round && !roundResolved && (
						<div className="round-timer" role="status" aria-live="polite">
							<div className="round-timer-top">
								<span>
									{playStartedAtMs === null ? "Timer ready" : "Time left"}
								</span>
							...
							{playStartedAtMs === null && (
								<p className="muted selector-help">
									Press Play to begin your countdown.
								</p>
							)}
						</div>
					)}
				...
				{round && !hasAnswered && !roundResolved && !isRoundTimeUp && (
					<div
						className="title-matches matches-board"
						role="listbox"
					...
					>
						...
						{matchingTitles.length === 0 && (
							<p className="muted selector-help">
								No matching titles in CSV cache.
							</p>
						)}
					</div>
				)}

				{round && !roundResolved && isRoundTimeUp && (
					<p className="muted selector-help">Time is up for this opening.</p>
				)}
				...
				{validationMessage && (
					<p className="muted selector-help">{validationMessage}</p>
				)}
				{isCorrect !== null && (
				...
				<div
					className="title-matches scoreboard-board"
					role="table"
				...
				>
					...
					{scoreboard.length === 0 && (
						<p className="muted selector-help">No players in room.</p>
					)}
				</div>
			</section>
		</main>
