...
const ensureNextRoundOwner = () => {
	if (
		...
		roomState.players.has(roomState.nextRoundOwnerPlayerId)
	) {
...
const resolvePlayer = async (playerId: string) => {
	...
	const persisted = (await readPlayers()).find(
...
export const getRoomState = async (playerId?: string) => {
	...
	const nextRoundOwnerName = nextRoundOwnerPlayerId
		? (scoreboard.find((entry) => entry.playerId === nextRoundOwnerPlayerId)
				?.name ?? null)
		: null;
	const roundWinnerName = roundWinnerPlayerId
		? (scoreboard.find((entry) => entry.playerId === roundWinnerPlayerId)
				?.name ?? null)
		: null;
	...
	return {
		...
		hasAnswered:
			Boolean(playerId) && roomState.currentRound
				? roomState.currentRound.answeredPlayerIds.has(playerId as string)
				: false,
		roundResolved,
		roundWinnerName,
		canStartNextRound:
