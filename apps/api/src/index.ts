...
const logApi = (
	...
	const serializedDetails = Object.entries(details)
		...
		.join(" ");
	console.log(
		`[api] ${event}${serializedDetails ? ` ${serializedDetails}` : ""}`,
	);
};
...
app.post("/api/room/join", async (c) => {
	try {
		...
		if (error instanceof HttpError) {
			logApi("room.join.error", {
				status: error.status,
				message: error.message,
			});
			return c.json(
				{ error: error.message },
				{ status: error.status as 400 | 401 | 404 | 409 },
			);
		}
...
app.post("/api/room/next-round", async (c) => {
	try {
		...
		const response = await beginRound(
			payload.playerId,
			round,
			payload.roundDurationSeconds,
		);
...
app.get("*", async (c) => {
	...
	const safePath = candidatePath.startsWith(webDistPath)
		? candidatePath
